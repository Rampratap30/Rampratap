...
@Component({
  selector: 'app-tmo-cdn-multiple-shipto',
  templateUrl: './tmo-cdn-multiple-shipto.component.html',
  styleUrls: ['./tmo-cdn-multiple-shipto.component.css']
})
export class TmoCdnMultipleShiptoComponent {
  order_status = {
    '1': 'Order Received',
    '2': 'In Process',
    '3': 'Scheduling',
  ...
  ngOnInit() {
    this.route.queryParams.subscribe((params) => {
      if (params['filter']) {
        ...
        const lineDetails = response['data']['line_details'];
  
        // Group line_details by address
        const groupedAddresses: { [key: string]: any[] } = {};
        lineDetails.forEach((line: { ship_to_address1: any; ship_to_city: any; ship_to_state: any; ship_to_zip: any; }) => {
          const key = `${line.ship_to_address1}|${line.ship_to_city}|${line.ship_to_state}|${line.ship_to_zip}`;
          if (!groupedAddresses[key]) {
            groupedAddresses[key] = [];
          }
          groupedAddresses[key].push(line);
        });
  
        // Flatten grouped addresses and append the suffix only for multiple addresses
        this.line_details = [];
        Object.values(groupedAddresses).forEach((group: any[]) => {
          if (group.length > 1) {
            group.forEach((line, index) => {
              line.address_suffix = `${index + 1}/${group.length}`;
            });
          } else {
            group[0].address_suffix = ''; // No suffix for single addresses
          }
          this.line_details.push(...group);
        });
        ...
        this.site_reference = response['data']['additional_details']['site_reference'];
        this.installation_details = response['data']['additional_details']['installation_details'];
