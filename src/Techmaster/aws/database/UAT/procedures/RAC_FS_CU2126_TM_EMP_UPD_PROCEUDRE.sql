...
DECLARE	v_manager_id                INTEGER DEFAULT 0;
...
DECLARE eligibleRecords 
   ...
   AND (change_effective_date <=CURRENT_DATE or change_effective_date is null)
	...
	CALL RAC_FS_TM_WRITE_LOG_MESSAGES('RAC_FS_TM_EMP_UPD_PROCEUDRE','Begin Procedure');
	
	/*opening cursor of new employees*/
	OPEN eligibleRecords;
    
    CALL RAC_FS_TM_WRITE_LOG_MESSAGES('RAC_FS_TM_EMP_UPD_PROCEUDRE','Cursor allRecords Opened');	
    ...
    allRecords: LOOP
		...
		FETCH eligibleRecords INTO v_change_id,
		   ...
		   v_change_status,
		   v_requested_by;
		CALL RAC_FS_TM_WRITE_LOG_MESSAGES('RAC_FS_TM_EMP_UPD_PROCEUDRE','Before Insert of New Employees');	
        
		/*exiting loop when no records found*/
		IF finished = 1 THEN 
        CALL RAC_FS_TM_WRITE_LOG_MESSAGES('RAC_FS_TM_EMP_UPD_PROCEUDRE','Leaving allRecords loop');
			LEAVE allRecords;
		END IF;
		...
		CALL RAC_FS_TM_WRITE_LOG_MESSAGES('RAC_FS_TM_EMP_UPD_PROCEUDRE',concat('After Update of change_id in tech master table:',v_change_id));
		...
		CALL RAC_FS_TM_WRITE_LOG_MESSAGES('RAC_FS_TM_EMP_UPD_PROCEUDRE',concat('After Update of change_id status to complete',v_change_id));
 ...
 CALL RAC_FS_TM_WRITE_LOG_MESSAGES('RAC_FS_TM_EMP_UPD_PROCEUDRE','End of Procedure');	
