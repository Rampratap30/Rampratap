DELIMITER //
CREATE PROCEDURE RAC_FS_TM_CSA_FLAG_PROCEDURE(IN P_IN_CHANGE_ID BIGINT(20))
BEGIN
...
DECLARE v_employee_name             VARCHAR(160); 
DECLARE v_fs_employee_name          VARCHAR(160);
DECLARE v_employee_name_chk         VARCHAR(10);
...
DECLARE v_fs_absence_start_date     TIMESTAMP;
DECLARE v_absence_start_date_chk    VARCHAR(10);
DECLARE v_absence_end_date          TIMESTAMP;
DECLARE v_fs_absence_end_date       TIMESTAMP;
DECLARE v_absence_end_date_chk     VARCHAR(10);
DECLARE v_actual_return_to_work     TIMESTAMP;
DECLARE v_fs_actual_return_to_work  TIMESTAMP;
DECLARE v_actual_return_to_work_chk VARCHAR(10);
...
DECLARE changeRecord
   CURSOR FOR 
	SELECT distinct
		...
		up.employee_name,
		fs.employee_name,
		(select csa_notification_flag from RAC_FS_TM_REQ_CLMS where field_name ='EMPLOYEE_NAME'),
		up.job_adp,
		(select job_adp from RAC_FS_TM_JOB_CODE job where job.job_id = fs.job_id),
		(select csa_notification_flag from RAC_FS_TM_REQ_CLMS where field_name ='JOB_ADP_CODE'),
		up.job_title,
		...
		up.manager_id,
		fs.manager_id,
		(select csa_notification_flag from RAC_FS_TM_REQ_CLMS where field_name ='MANAGER_EMPLOYEE_ID'),
		up.manager_flag,
		fs.manager_flag,
		...
		(select csa_notification_flag from RAC_FS_TM_REQ_CLMS where field_name ='ABSENCE_START_DATE'),
		up.absence_end_date,
		fs.absence_end_date,
		(select csa_notification_flag from RAC_FS_TM_REQ_CLMS where field_name ='ABSENCE_END_DATE'),
		up.actual_return_to_work,
		fs.actual_return_to_work,
		(select csa_notification_flag from RAC_FS_TM_REQ_CLMS where field_name ='ACTUAL_RETURN_TO_WORK')
	FROM
    ...
    getDetails: LOOP
		...
		FETCH changeRecord 
		 ...
		 v_fs_actual_return_to_work,
		 v_actual_return_to_work_chk;
     ...
     /*exiting loop when no records found*/
	    ...
	    If v_absence_start_date = '0000-00-00 00:00:00'
		THEN
		   set v_absence_start_date = NULL;
		END IF;
		
		If v_fs_absence_start_date = '0000-00-00 00:00:00'
		THEN
		   set v_fs_absence_start_date = NULL;
		END IF;
		
		If v_absence_end_date = '0000-00-00 00:00:00'
		THEN
		   set v_absence_end_date = NULL;
		END IF;
		
		If v_fs_absence_end_date = '0000-00-00 00:00:00'
		THEN
		   set v_fs_absence_end_date = NULL;
		END IF;
		
		If v_actual_return_to_work = '0000-00-00 00:00:00'
		THEN
		   set v_actual_return_to_work = NULL;
		END IF;
		
		If v_fs_actual_return_to_work = '0000-00-00 00:00:00'
		THEN
		   set v_fs_actual_return_to_work = NULL;
		END IF;
	...
	END IF;*/
	 ...
	 IF v_alternate_email != v_fs_alternate_email AND v_alternate_email_chk ='Y'
	THEN
      UPDATE RAC_FS_TM_EMPLOYEE_UPD
	     SET CSA_NOTIFICATION_REQUIRED = 'Y',CSA_CHANGE_COMMENT=CONCAT(CSA_CHANGE_COMMENT,'Cell Number. ')
	   WHERE CHANGE_ID = P_IN_CHANGE_ID;
	...
	IF v_manager_id != v_fs_manager_id  AND v_manager_chk ='Y'
		THEN
      UPDATE RAC_FS_TM_EMPLOYEE_UPD
	     SET CSA_NOTIFICATION_REQUIRED = 'Y',CSA_CHANGE_COMMENT=CONCAT(CSA_CHANGE_COMMENT,'Manager Change. ')
	   WHERE CHANGE_ID = P_IN_CHANGE_ID;
	...
	IF v_employee_name != v_fs_employee_name  AND v_employee_name_chk ='Y'
		THEN
      UPDATE RAC_FS_TM_EMPLOYEE_UPD
	     SET CSA_NOTIFICATION_REQUIRED = 'Y',CSA_CHANGE_COMMENT=CONCAT(CSA_CHANGE_COMMENT,'Employee Name change. ')
	   WHERE CHANGE_ID = P_IN_CHANGE_ID;
	...
	IF (COALESCE(DATE_FORMAT(v_absence_start_date,"%Y-%m-%d"),'X') != COALESCE(DATE_FORMAT(v_fs_absence_start_date,"%Y-%m-%d"),'X') AND v_absence_start_date_chk='Y') 
	THEN
	   UPDATE RAC_FS_TM_EMPLOYEE_UPD
	     SET CSA_NOTIFICATION_REQUIRED = 'Y',CSA_CHANGE_COMMENT=CONCAT(CSA_CHANGE_COMMENT,'Absence Start Date. ')
	   WHERE CHANGE_ID = P_IN_CHANGE_ID;
	...
	IF (COALESCE(DATE_FORMAT(v_absence_end_date,"%Y-%m-%d"),'X') != COALESCE(DATE_FORMAT(v_fs_absence_end_date,"%Y-%m-%d"),'X') AND v_absence_end_date_chk='Y') 
	THEN
	   UPDATE RAC_FS_TM_EMPLOYEE_UPD
	     SET CSA_NOTIFICATION_REQUIRED = 'Y',CSA_CHANGE_COMMENT=CONCAT(CSA_CHANGE_COMMENT,'Absence End Date. ')
	   WHERE CHANGE_ID = P_IN_CHANGE_ID;
	   
      COMMIT;	  
	END IF;
	
	/*IF (COALESCE(v_actual_return_to_work,'X') != COALESCE(v_fs_actual_return_to_work,'X') AND v_actual_return_to_work_chk='Y') 
	THEN
	   UPDATE RAC_FS_TM_EMPLOYEE_UPD
	     SET CSA_NOTIFICATION_REQUIRED = 'Y',CSA_CHANGE_COMMENT=CONCAT(CSA_CHANGE_COMMENT,'Actual Return To Work. ')
	   WHERE CHANGE_ID = P_IN_CHANGE_ID;
