...
DECLARE v_employee_id               BIGINT(60);
...
DECLARE v_fs_manager_id             BIGINT(60) DEFAULT 0;
   ...
   CURSOR FOR 
	SELECT 
       ...
       # up.review_date,
        ...
        up.absence_end_date,
        fs.absence_end_date,
        (select approval_req from RAC_FS_TM_REQ_CLMS where field_name ='ABSENCE_END_DATE' and up.absence_end_date IS NOT NULL),
	...
	(COALESCE(v_region,'X') != COALESCE(v_fs_region,'X') AND v_region_chk='Y') 
	 ...
	 OR ((COALESCE(DATE_FORMAT(v_absence_start_date,"%Y-%m-%d"),'X') != COALESCE(DATE_FORMAT(v_fs_absence_start_date,"%Y-%m-%d"),'X')) AND v_absence_start_date_chk ='Y')
     OR ((COALESCE(DATE_FORMAT(v_absence_end_date,"%Y-%m-%d"),'X') != COALESCE(DATE_FORMAT(v_fs_absence_end_date,"%Y-%m-%d"),'X')) AND v_absence_end_date_chk ='Y')
	 OR ((COALESCE(DATE_FORMAT(v_return_to_work,"%Y-%m-%d"),'X') != COALESCE(DATE_FORMAT(v_fs_return_to_work,"%Y-%m-%d"),'X')) AND v_return_to_work_chk ='Y')
    THEN 
